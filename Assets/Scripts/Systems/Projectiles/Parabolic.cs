...
//
//    bool IsZero(float d);
//    int SolveQuadric(float c0, float c1, float c2, out float s0, out float s1);
//    int SolveCubic(float c0, float c1, float c2, float c3, out float s0, out float s1, out float s2);
//    int SolveQuartic(float c0, float c1, float c2, float c3, float c4, out float s0, out float s1, out float s2, out float s3);
...
{ 
    ...
    public static int SolveCubic(float c0, float c1, float c2, float c3, out float s0, out float s1, out float s2)
    {
        ...
        sq_A = A * A;
        p = 1.0f / 3f * (-1.0f / 3f * sq_A + B);
        q = 1.0f / 2f * (2.0f / 27f * A * sq_A - 1.0f / 3f * A * B + C);
        ...
        {
            float phi = 1.0f / 3f * Mathf.Acos(-q / Mathf.Sqrt(-cb_p));
    ...
    public static int SolveQuartic(float c0, float c1, float c2, float c3, float c4, out float s0, out float s1, out float s2, out float s3)
    {
        ...
        sq_A = A * A;
        p = -3.0f / 8f * sq_A + B;
        q = 1.0f / 8f * sq_A * A - 1.0f / 2f * A * B + C;
        r = -3.0f / 256f * sq_A * sq_A + 1.0f / 16f * sq_A * B - 1.0f / 4f * A * C + D;
        ...
        else
        {
            ...
            coeffs[3] = 1.0f / 2f * r * p - 1.0f / 8f * q * q;
            coeffs[2] = -r;
            coeffs[1] = -1.0f / 2f * p;
            coeffs[0] = 1;
    ...
    public static Vector3 PositionAtTime(Vector3 start, Vector3 velocity, float gravity, float time)
    {
        return new Vector3(
            x: start.x + (velocity.x * time), // + (.5f * 0 * time * time),
            y: start.y + (velocity.y * time) + (.5f * gravity * time * time),
            z: start.z + (velocity.z * time)); // + (.5f * 0 * time * time));
    }
}